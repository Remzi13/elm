...
struct AABB {
    ...
    constexpr AABB(const Vector3& inMin, const Vector3& inMax)
        : minBounds(inMin)
        , maxBounds(inMax)
    {
    }
    ...
    [[nodiscard]] bool operator==( const AABB& o ) const noexcept
    {
        return minBounds == o.minBounds && maxBounds == o.maxBounds;
    }
};
...
struct Vertex {
    ...
    {
        return position == oth.position && normal == oth.normal && u == oth.u && v == oth.v;
    }
};
...
struct MeshData {
    ...
    bool operator==(const MeshData& oth) const
    {
        return vertices == oth.vertices && indices == oth.indices && localBounds == oth.localBounds;
    }
};
